...
vector<unsigned char> WINFUNC::browseUC(string url)
{
	...
	if (cut_here > 0 && cut_here < temp.size()) { url = temp.substr(cut_here); }
	for (int ii = 0; ii < domains.size(); ii++)
	{
		cut_here = url.rfind(domains[ii]);
		if (cut_here < url.size())
...
wstring WINFUNC::browseW(string url)
{
	...
	if (cut_here < temp.size()) { url = temp.substr(cut_here); }
	for (int ii = 0; ii < domains.size(); ii++)
	{
		cut_here = url.rfind(domains[ii]);
		if (cut_here < url.size())
...
vector<int> WINFUNC::getDateTime()
{
	// Return form [year, month, day of week, day of month, hour, minute, second, millisecond].
	// Day of week has form 0 = Sunday, ..., 6 = Saturday.
	SYSTEMTIME local;
	GetLocalTime(&local);
	vector<int> viDateTime(8);
	viDateTime[0] = (int)local.wYear;
	viDateTime[1] = (int)local.wMonth;
	viDateTime[2] = (int)local.wDayOfWeek;
	viDateTime[3] = (int)local.wDay;
	viDateTime[4] = (int)local.wHour;
	viDateTime[5] = (int)local.wMinute;
	viDateTime[6] = (int)local.wSecond;
	viDateTime[7] = (int)local.wMilliseconds;
	return viDateTime;
}
string WINFUNC::get_exec_path()
{
	string execPath = getExecPath(0);
	return execPath;
}
string WINFUNC::getExecPath(int folderUp)
{
	// folderUp specifies the number of steps towards the root returned from the executable.
	string execPath;
	LPSTR bufferA = new CHAR[500];
	DWORD slength = GetModuleFileNameA(NULL, bufferA, 500);
	if (slength == 0) { winerr("GetModuleFileName-wf.getExecPath"); }
	execPath.assign(bufferA, slength);
	delete[] bufferA;
	if (folderUp == 0) { return execPath; }

	size_t pos1 = execPath.size();
	for (int ii = 0; ii < folderUp; ii++) {
		pos1 = execPath.rfind('\\', pos1 - 1);
		if (pos1 > execPath.size()) { jf.err("folderUp exceeds directory length-wf.getExecPath"); }
	}

	string rootFolder = execPath.substr(0, pos1);
	return rootFolder;
}
...
void WINFUNC::init()
{
	domains = { ".com", ".net", ".org", ".edu", ".ca", ".info", ".co", ".de" };
}
