...
MainWindow::MainWindow(wstring execFolder, QWidget *parent)
    : QMainWindow(parent), wsExecFolder(execFolder), ui(new Ui::MainWindow)
{
    ui->setupUi(this);
...
void MainWindow::initGUI()
{
    ...
    {
        pos2 = savedSettings.find("\n\n", pos1);
        if (pos2 > savedSettings.size()) { err("Parsing navSearch-MainWindow.initGUI"); }
    }
    ...
    // Local catalogue tree.
    ui->qjTree_catalocal->initCol({ {"sValue", "Name"} });
...
{
    string settingsPath = jf.utf16To8(wsExecFolder) + "\\SCDA-Explorer Settings.txt";
    savedSettings = jf.load(settingsPath);
    size_t pos1 = savedSettings.find("//dbPath");
    pos1 = savedSettings.find('\n', pos1) + 1;
    size_t pos2 = savedSettings.find('\n', pos1);
    string dbPath = savedSettings.substr(pos1, pos2 - pos1);

    // Adjust the resolution.
    on_pB_resolution_clicked();

    // Initialize the class objects.
    sb.setErrorPath(jf.utf16To8(wsExecFolder) + "\\SCDA SWITCHBOARD Error Log.txt");
    sf.init(db_path);
...
void MainWindow::barUpdate(int iCurrent)
{
    ...
    if (iCurrent < min || iCurrent > max) { err("Outside boundaries-MainWindow.barUpdate"); }
    ui->progressbar->setValue(iCurrent);
}
void MainWindow::barReset(int iMax, string message)
{
    if (iMax < 0) { err("Invalid maximum-MainWindow.barReset"); }
...
void MainWindow::bind(string& stmt, vector<string>& param)
{
    ...
    if (count != param.size())
    {
        err("parameter count mismatch-MainWindow.bind");
    }
...
void MainWindow::err(string message)
{
    string errorMessage = "MainWindow error:\n" + message;
    JLOG::getInstance()->err(errorMessage);
}
void MainWindow::log(string message)
{
    string logMessage = "MainWindow log entry:\n" + message;
...
void MainWindow::thrDownload(SWITCHBOARD& sbgui)
{
    ...
    catch (invalid_argument) { err("stoi-MainWindow.thrDownload"); }
...
void MainWindow::thrFileSplitter(SWITCHBOARD& sbgui, int& progress, mutex& m_progress)
{
    ...
    catch (invalid_argument) { err("stoll-MainWindow.thrFileSplitter"); }
...
void MainWindow::scanLocalCata(SWITCHBOARD& sbgui, JTREE& jtgui)
{
    ...
    jf.sortInteger(iYearList, JFUNC::Increasing);
    int numYear = iYearList.size();
    sYearList.resize(numYear);
    for (int ii = 0; ii < numYear; ii++) {
        sYearList[ii] = to_string(iYearList[ii]);
        JNODE jnYear = jtgui.newNode();
        jnYear.sValue = sYearList[ii];
        jtgui.addChild(rootID, jnYear);
    }
    
    vector<JNODE&> vJNYear = jtgui.getChildren(rootID);
    for (int ii = 0; ii < numYear; ii++) {
        parentID = vJNYear[ii].ID;
        ...
        for (int jj = 0; jj < numCata; jj++) {
            ...
            if (meta == 1 && csvCount[jj] > 0) {
                JNODE jn = jtgui.newNode();
                jn.sValue = folderList[jj];
                jtgui.addChild(parentID, jn);
            }
        }
    }   
    mycomm[0] = 1;
...
void MainWindow::updateDBCata()
{
    ...
    jf.sortInteger(yearList, JFUNC::Increasing);
...
void MainWindow::judicator(SWITCHBOARD& sbgui, SQLFUNC& sfgui)
{
    ...
    sfgui.executor(result);
    log("Added " + prompt[0] + " to the Census table.");
    ...
    sfgui.executor(result);
    log("Added " + prompt[1] + " to the CensusYear table.");
    ...
    log("Added " + prompt[1] + " to the 'topic' table.");

    // Create the catalogue's geo table. Rows are added later, with the CSV data.
    result = scjudi.makeCreateGeo();
    sfgui.executor(result);
    log("Created a geo table for " + prompt[1]);

    // Create the DIM/dim tables.
    vsResult = scjudi.makeCreateInsertDIMIndex();
    sfgui.executor(vsResult);
    log("Created and inserted a DIMIndex table for " + prompt[1]);
    if (vsResult.size() > 1)
    {
        vsResult = scjudi.makeCreateInsertDim();
        sfgui.executor(vsResult);
        log("Created and inserted a dim table for " + prompt[1]);
    }
    if (vsResult.size() > 2)
    {
        vsResult = scjudi.makeCreateInsertDIM(vvsDIM);
        sfgui.executor(vsResult);
        log("Created and inserted DIM tables for " + prompt[1]);
    }
    else { log("Did not make DIM/dim tables for " + prompt[1]); }
    ...
    if (!sfgui.tableExist(tname))
    {
        ...
        sfgui.executor(result);
        log("Created a DataIndex table for " + prompt[1]);
    }
    search = { "DataIndex" };
    vsResult.clear();
    sfgui.select(search, tname, vsResult);
    ...
    if (numStmt > 0)
    {
        ...
        delete insertDI;
        log("Inserted " + to_string(numStmt) + " DataIndex rows for " + prompt[1]);
    }
    ...
    delete vsCreateData;
    log("Created data table statements for " + prompt[1]);
    ...
    if (geoToDo.size() > 0)
    {
        ...
        else { err("mapGeoCode-MainWindow.judicator"); }
    ...
    thr3->join();
    delete thr3;
    log("Inserted all data for " + prompt[1]);
    ...
    for (int ii = 0; ii < vsResult.size(); ii++)
    {
        ...
        catch (invalid_argument) { err("stoi-scjudi.makeInsertData"); }
    ...
    if (needCol > 0)
    {
        ...
        log("Added ancestor columns into " + tname);
    }
    ...
    for (int ii = 0; ii < viGeoCode.size(); ii++)
    {
        ...
        catch (invalid_argument) { err("stoi-MainWindow.judicator"); }
    ...
    log("Inserted ancestry values into " + tname);

    // Geo Layers.
    insertGeoLayers(prompt[0], prompt[1]);
    log("Inserted geo layers for " + prompt[1]);
    ...
    if (!sfgui.tableExist(tname))
    {
        ...
        sfgui.executor(result);
        log("Created a ForWhom table for " + prompt[0]);
    }
    stmt = scjudi.makeInsertForWhom();
    sfgui.executor(stmt);
    log("Inserted a ForWhom value for " + prompt[1]);
...
void MainWindow::makeInsertDataThr(SWITCHBOARD& sbjudi, SQLFUNC& sfgui)
{
    ...
    if (!wf.file_exist(geoPath)) { err("Missing Geo.txt-MainWindow.makeInsertDataThr"); }
...
unordered_map<string, string> MainWindow::makeMapDataIndex(SQLFUNC& sfgui, string tname)
{
    ...
    if (vvsResult.size() < 1) { err("Table is empty-sf.makeMapDataIndex"); }
...
void MainWindow::createBinMap(SWITCHBOARD& sbgui)
{
    ...
    for (int ii = 0; ii < pngPath.size(); ii++)
    {
        ...
        if (!bm.checkSpec(imgSpec)) { err("Incorrect PNG resolution-MainWindow.createBinMap"); }
        binPath = pngPath[ii];
        ...
        else
        {
            ...
            catch (invalid_argument) { err("stod-MainWindow.createBinMap"); }
        }
        ...
        if (!position) {
            ...
            pos1 = binFile.find('\n', pos1) + 1;
            temp = binFile.substr(pos1, pos2 - pos1);
            jf.destringifyCoord(temp, centerFromHomeKM);
        }
        wf.printer(binPath, binFileNew);
        ...
        if (!frame) {
            ...
            pos2 = binFile.find('\n', pos1);
            temp = binFile.substr(pos1, pos2 - pos1);
            jf.destringifyCoord(temp, frameTLBR[0]);
            pos1 = pos2 + 1;
            pos2 = binFile.find('\n', pos1);
            temp = binFile.substr(pos1, pos2 - pos1);
            jf.destringifyCoord(temp, frameTLBR[1]);
        }
        wf.printer(binPath, binFileNew);
        ...
        if (!border)
        {
            ...
            {
                bool painted = bm.initPainted(pngPath[ii]);
                if (!painted) { err("No paintedPNG-bm.createBinMap"); }
                tempTLBR = bm.paintRegionFrame();
            }
...
void MainWindow::getMapDBCoord(SWITCHBOARD& sbgui, SQLFUNC& sfgui, vector<vector<vector<double>>>& coord)
{
    ...
    for (int ii = 0; ii < coord.size(); ii++)
    {
        ...
        catch (invalid_argument) { err("stod-MainWindow.getMapDBCoord"); }
...
void MainWindow::on_pB_reviewmap_clicked()
{
    ...
    else if (qTemp == "treeW_mapdb")
    {
        ...
        catch (out_of_range) { err("mapCataToYear-MainWindow.on_pB_reviewmap_clicked"); }
        sb.set_prompt(prompt);
...
void MainWindow::getCataMapDB(SWITCHBOARD& sbgui, SQLFUNC& sfgui, JTREE& jtgui)
{
    ...
    for (int ii = 0; ii < geo.size(); ii++)
    {
        ...
        catch (invalid_argument) { err("stoi-MainWindow.getCataMapDB"); }
...
void MainWindow::insertCataMaps(SWITCHBOARD& sbgui, SQLFUNC& sfgui)
{
    ...
    for (int ii = 0; ii < geo.size(); ii++)
    {
        ...
        if (!sfgui.tableExist(tname))
        {
            ...
            try { geoLevel = stoi(geo[ii][2]); }
            catch (invalid_argument) { err("stoi-MainWindow.insertCataMaps"); }
...
void MainWindow::on_pB_deletemap_clicked()
{
    ...
    if (qTemp == "treeW_mapdb")
    {
        ...
        catch (out_of_range) { err("mapCataToYear-MainWindow.on_pB_deletemap_clicked"); }
...
void MainWindow::displayTable(QTableWidget*& qTable, string tname)
{
    ...
    sb.end_call(myid);
    if (vvsResult.size() < 1) { err("No rows found-MainWindow.displayTable"); }
...
void MainWindow::on_pB_deletetable_clicked()
{
    ...
    else { err("Failed to read button-MainWindow.on_pB_deletetable_clicked"); }
    switch (mode)
    {
    case 0:
    {
...
void MainWindow::downloadCatalogue(string sYear, string sCata)
{
    ...
    if (!wf.file_exist(prompt[1])) { err("Downloaded file not found-MainWindow.on_pB_download_clicked"); }
    size_t pos1 = prompt[1].rfind('\\');
    string folderPath = prompt[1].substr(0, pos1);
    ...
    if (!wf.file_exist(csvPath)) { err("Unzipped file not found-MainWindow.on_pB_download_clicked"); }
...
void MainWindow::on_pB_test_clicked()
{
    ...
    case 0:  // Download the webpage given by the text box's URL.
    {
        ...
        wstring wsPath = wsExecFolder + L"\\Test webpage.txt";
        jf.printer(wsPath, webpage);
        qtemp = "Done!";
        ui->pte_search->setPlainText(qtemp);
        return;
    }
    ...
    case 3:  
    {
        ...
        for (int ii = 0; ii < vsCata.size(); ii++) {
            ...
            while (pos2 < csvFile.size()) {
                ...
                catch (invalid_argument) { err("stoi-test3:" + temp); }
                temp = to_string(inum);
                if (temp.size() > 5) {
                    pos2 = csvFile.find("\r\n", pos2);
                    pos2 = csvFile.find(',', pos2);
    ...
    case 4:  // Insert a complete geo table using the given tnameGeo.
    {
        ...
        for (int ii = 0; ii < geoData.size(); ii++)
        {
            ...
            catch (invalid_argument) { err("stoi-MainWindow.test4"); }
            if (inum > maxLevel) { maxLevel = inum; }
        }
...
void MainWindow::convertGuide(SWITCHBOARD& sbgui, QPainterPath& painterPathBorder, vector<string>& pathBIN)
{
    ...
    try { inum = stoi(temp); }
    catch (invalid_argument& ia) { err("stoi-MainWindow.convertGuide"); }
    im.setPauseVBP(inum);
    pos1++;
...
void MainWindow::convertSingle(SWITCHBOARD& sbgui)
{
    ...
    catch (invalid_argument) { err("stoi-MainWindow.convertSingle"); }
    im.setExtractDim(windowDim);
