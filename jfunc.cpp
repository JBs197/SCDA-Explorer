...
vector<unsigned char> JFUNC::baseConvert(int inputWidth, vector<unsigned char>& input, int x, int y)
{
	// Converts a chunk of data from base 'x' to base 'y'.
	// sourceWidth indicates how many vector elements are to be grouped together.
	if (x < 2 || y < 2 || x > 256 || y > 256) { err("Invalid base-jf.baseConvert"); }
	if (input.size() % inputWidth != 0) { err("Uneven input division-jf.baseConvert"); }
	int baseTenMax = 0;
	for (int ii = 0; ii < inputWidth; ii++) {
		baseTenMax += ((x - 1) * pow(x, ii));
	}
	int outputWidth = 0;
	int quotient = baseTenMax;
	do {
		quotient /= y;
		outputWidth++;
	} while (quotient > 0);
	...
	int baseTen;
	size_t startInput, startOutput;
	size_t numGroup = input.size() / inputWidth;
	size_t lenOutput = numGroup * outputWidth;
	vector<unsigned char> output(lenOutput);
	for (size_t ii = 0; ii < numGroup; ii++) {
		baseTen = 0;
		startInput = ii * inputWidth;
		for (int jj = 0; jj < inputWidth; jj++) {
			baseTen += (input[startInput + jj] * pow(x, inputWidth - jj - 1));
		}
		startOutput = ii * outputWidth;
		for (int jj = 0; jj < outputWidth; jj++) {
			output[startOutput + jj] = baseTen / pow(y, outputWidth - jj - 1);
			baseTen -= (output[startOutput + jj] * pow(y, outputWidth - jj - 1));
		}
	}
	return output;
}
...
vector<int> JFUNC::destringifyCoord(string& sCoord)
{
	...
	while (pos1 < sCoord.size())
	{
		...
		catch (invalid_argument) { err("stoi-jf.destringifyCoord"); }
		pos2 = pos1 + 1;
...
void JFUNC::hexDec(string& sHex, int& iDec)
{
	if (sHex.size() < 1) {
		sHex = decToHex(iDec);
	}
...
void JFUNC::hexToDec(string sHex, int& iDec)
{
	if (sHex[0] == '#') { sHex.erase(sHex.begin()); }
	int len = sHex.size();
	iDec = 0;
	double dnum;
	string temp;
	for (int ii = 0; ii < len; ii++) {
		if (sHex[ii] == 'F' || sHex[ii] == 'f') { dnum = 15.0; }
		else if (sHex[ii] == 'E' || sHex[ii] == 'e') { dnum = 14.0; }
		else if (sHex[ii] == 'D' || sHex[ii] == 'd') { dnum = 13.0; }
		else if (sHex[ii] == 'C' || sHex[ii] == 'c') { dnum = 12.0; }
		else if (sHex[ii] == 'B' || sHex[ii] == 'b') { dnum = 11.0; }
		else if (sHex[ii] == 'A' || sHex[ii] == 'a') { dnum = 10.0; }
		else { 
			temp = sHex[ii];
			try { dnum = stod(temp); }
			catch (invalid_argument) { err("stod-jf.hexToDec"); }
		}

		iDec += int(dnum * pow(16.0, (double)(len - ii - 1)));
	}
}
...
{
	int isize = (int)ilist.size(), index;
	vector<int> viTemp;
	switch (type)
	{
	case 0:
...
void JFUNC::isort_ilist(vector<string>& slist, int type)
{
	...
	int isize = (int)slist.size(), index;
...
void JFUNC::sortAlphabetically(vector<vector<string>>& vvsList, int iCol)
{
	...
	size_t sizeList = vvsList.size();
	if (sizeList < 2) { return; }
	int count = 1, comparison;
	while (count > 0)
	{
		count = 0;
		for (int ii = 0; ii < sizeList - 1; ii++)
		{
...
void JFUNC::sortAlphabetically(vector<vector<string>>& vvsList, vector<int> viSort)
{
	...
	int topIndex = (int)viSort.size() - 1;
...
vector<int> JFUNC::svectorToIvector(vector<string>& svec)
{
	...
	for (int ii = 0; ii < svec.size(); ii++)
	{
		...
		catch (invalid_argument) { err("stoi-jfunc.svectorToIvector"); }
	}
	return ivec;
}
...
void JFUNC::uptick(vector<int>& viCounter, vector<int> viMax)
{
	...
	for (int ii = (int)viCounter.size() - 1; ii >= 0; ii--)
	{
