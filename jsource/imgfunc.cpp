...
void CANDIDATES::err(string message)
{
    string errorMessage = "CANDIDATES error:\n" + message;
    JLOG::getInstance()->err(errorMessage);
}
int CANDIDATES::fromCircle(POINT pCenter, int radius)
{
    if (scanCircles.size() < 1) { err("No init for scanCircles-cd.fromCircle"); }
    if (radius < 1 || radius > scanCircles.size()) { err("Bad radius-cd.fromCircle"); }
...
{
    if (pList.size() != vdDist.size()) { err("Parameter size mismatch-cd.judgeDist"); }
...
int CANDIDATES::removeColourRatio(int ratio, vector<double> vdInterval)
{
    ...
    if (vdInterval.size() < 2) { err("No interval given-cd.removeColourRatio"); }
    double dRatio;
    for (int ii = rgbaList.size() - 1; ii >= 0; ii--)
    {
        switch (ratio)
...
int CANDIDATES::removeRearCone(vector<POINT>& vpPast)
{
    ...
    if (vpPast.size() < 3) { err("Insufficient vpPast length-cd.removeRearCone"); }
...
vector<double> CANDIDATES::reportDist(vector<POINT>& vpPast)
{
    ...
    if (vpPast.size() < 3) { err("Insufficient vpPast length-cd.reportDist"); }
...
string IMGFUNC::convertColour(vector<unsigned char>& rgbx)
{
    ...
    for (int ii = 0; ii < rgbx.size(); ii++)
    {
        ...
        catch (out_of_range) { err("mapDecHex-im.convertColour"); }
        srgbx.append(1, quot);
        srgbx.append(1, rem);
    }
    return srgbx;
...
vector<unsigned char> IMGFUNC::convertColour(string& srgbx)
{
    ...
    else { err("Parameter size-im.convertColour"); }
    unsigned char quotient, remainder;
    for (int ii = 0; ii < rgbx.size(); ii++)
    {
        try
        ...
        catch (out_of_range) { err("mapHexDec-im.convertColour"); }
...
void IMGFUNC::crop(vector<unsigned char>& img, vector<int>& imgSpec, vector<POINT> TLBR)
{
    ...
    if (img.size() < 1 || imgSpec.size() < 1 || TLBR.size() < 2) { err("Bad input parameters-im.crop"); }
...
void IMGFUNC::cropSave(vector<string>& filePath, vector<POINT> TLBR)
{
    ...
    if (filePath.size() < 2 || TLBR.size() < 2) { err("Bad input parameters-im.cropSave"); }
...
void IMGFUNC::err(string message)
{
    string errorMessage = "IMGFUNC error:\n" + message;
    JLOG::getInstance()->err(errorMessage);
}
...
void IMGFUNC::flatten(string& filePath)
{
    ...
    else { err("Unknown bits-per-pixel-im.flatten"); }
}
...
POINT IMGFUNC::getBorderPoint(vector<unsigned char>& img, vector<int>& imgSpec, vector<POINT> startStop, vector<vector<double>> vvdInside)
{
    ...
    if (startStop[0].x < 0 || startStop[1].x < 0) { err("startStop out of bounds-im.getBorderPoint"); }
    if (startStop[0].y < 0 || startStop[1].y < 0) { err("startStop out of bounds-im.getBorderPoint"); }
    if (startStop[0].x >= imgSpec[0] || startStop[1].x >= imgSpec[0]) { err("startStop out of bounds-im.getBorderPoint"); }
    if (startStop[0].y >= imgSpec[1] || startStop[1].y >= imgSpec[1]) { err("startStop out of bounds-im.getBorderPoint"); }
    CANDIDATES cd;
    initCandidates(cd);
...
POINT IMGFUNC::getBorderPoint(vector<unsigned char>& img, vector<int>& imgSpec, POINT pStart, double angle, vector<POINT> TLBR, vector<vector<double>> vvdInside)
{
    ...
    if (angle < 0.0 || angle >= 360.0) { err("Invalid angle-im.getBorderPoint"); }
    if (pStart.x < 0 || pStart.x >= imgSpec[0]) { err("Invalid pStart-im.getBorderPoint"); }
    if (pStart.y < 0 || pStart.y >= imgSpec[1]) { err("Invalid pStart-im.getBorderPoint"); }
...
vector<POINT> IMGFUNC::getCircle(POINT pCenter, int radius)
{
    ...
    if (scanCircles.size() < 1) { err("No init for scanCircles-im.getCircle"); }
    if (radius < 1 || radius >= scanCircles.size()) { err("Bad radius-im.getCircle"); }
...
{
    if (rgbaList.size() < 1) { err("No size specified-im.getColourSpectrum"); }
    if (keyColour.size() < 1)
    {
        keyColour.resize(6);
...
double IMGFUNC::getGB(vector<unsigned char>& rgbx)
{
    ...
    if (rgbx.size() < 3) { err("No init colour-im.getGB"); }
...
double IMGFUNC::getRB(vector<unsigned char>& rgbx)
{
    ...
    if (rgbx.size() < 3) { err("No init colour-im.getRB"); }
...
double IMGFUNC::getRG(vector<unsigned char>& rgbx)
{
    ...
    if (rgbx.size() < 3) { err("No init colour-im.getRG"); }
...
{
    if (scanCircles.size() < 1) { err("No scanCircles init-im.initCandidates"); }
    cd.initialize(scanCircles, pScanCircleCenter);
}
...
vector<POINT> IMGFUNC::loadBox(vector<unsigned char>& img, vector<int>& imgSpec, vector<unsigned char> RGBX)
{
    ...
    if (RGBX.size() != imgSpec[2]) { err("rgbx parameter mismatch-im.loadBox"); }
    vector<POINT> TLBR(2);
...
void IMGFUNC::pngCanvas(vector<int>& imgSpec, vector<unsigned char>& img, vector<unsigned char> rgbx)
{
    ...
    if (imgSpec.size() < 3) { err("No imgSpec given-im.pngCanvas"); }
    if (rgbx.size() < 3) { err("No rgbx given-im.pngCanvas"); }
...
void IMGFUNC::pngLoadHere(string& pngPath, vector<unsigned char>& pngData, vector<int>& spec)
{
    ...
    if (dataTemp == NULL) { err("stbi_load-im.pngLoadString"); }
...
void IMGFUNC::scanPatternLineH(vector<unsigned char>& img, vector<int>& imgSpec, vector<vector<vector<unsigned char>>>& colourList, vector<int>& viResult, vector<vector<int>> startStop)
{
    ...
    if (colourList[0].size() != colourList[1].size()) { err("colourList size mismatch-im.scanPatternLineH"); }
    vector<unsigned char> rgbx;
    unordered_map<string, int> mapColour;
...
int IMGFUNC::areaRect(vector<vector<int>> TLBR)
{
    ...
    if (area < 0) { err("Negative area-im.areaRect"); }
    return area;
}
vector<int> IMGFUNC::borderFindNext(SWITCHBOARD& sbgui, vector<vector<int>> tracks)
{
    ...
    err("Failed to determine next border point-im.borderFindNext");
    return failure;
}
vector<int> IMGFUNC::borderFindStart()
{
    if (!isInit()) { err("No init-im.borderFindStart"); }
...
double IMGFUNC::clockwisePercentage(SWITCHBOARD& sbgui, vector<vector<int>>& tracks, string sZone)
{
    ...
    if (tracks.size() < 2) { err("Not enough coordinates-im.clockwisePercentage"); }
    int numClockwise = 0;
...
vector<int> IMGFUNC::coordStoi(string& sCoords)
{
	...
	catch (invalid_argument& ia) { err("stoi1-im.coordStoi"); }
	...
	catch (invalid_argument& ia) { err("stoi2-im.coordStoi"); }
	return vCoords;
}
...
double IMGFUNC::getPPKM(string& pathTXT, int scalePixels)
{
    ...
    FTL1:
        ...
        if (pos2 > sfile.size()) { err("Cannot find kilometer-im.getPPKM"); }
    ...
    int iKM;
    try { iKM = stoi(temp); }
    catch (invalid_argument& ia) { err("stoi-im.getPPKM"); }    
...
int IMGFUNC::getQuadrant(vector<vector<int>>& startStop)
{
    ...
    else
    {
        ...
        else { err("startStop identical-im.getQuadrant"); }
    }
    return -1;
}
int IMGFUNC::getScalePixels(vector<vector<int>> TLBR)
{
    ...
    if (sZone != "black") { err("Failed to measure wall thickness-im.getScalePixels"); }
    while (sZone == "black")
    {
        thickness++;
        coord[0]++;
...
double IMGFUNC::getStretchFactor(string& widthHeight)
{
    ...
    catch (invalid_argument& ia) { err("stod-im.getStretchFactor"); }
...
{
    if (savePoints.size() < 1) { err("No saved points to load-im.loadRecentSavePoint"); }
    ...
    for (int ii = savePoints.size() - 1; ii >= 0; ii--)
    {
        ...
        else
        {
            ...
            if (savePoints[ii].size() != 3) { err("savePoints-im.loadRecentSavePoint"); }
    ...
    if (pointRight.size() < 2) { err("Failed to determine pointRight-im.loadRecentSavePoint"); }
...
void IMGFUNC::makePositionPNG(string& pngPath, string& pngFramePath)
{
    ...
    delete[] bufferUC;
    if (!success) { err("stbi_write_png-im.makePositionPNG"); }
}
...
void IMGFUNC::pauseMapDebug(SWITCHBOARD& sbgui, vector<vector<int>>& tracks, int radius, vector<vector<int>>& candidates)
{
    if (pathMapDebug.size() > 0)
    {
        ...
        if (vsTemp.size() == 1)  // Advance to prompt's index.
        {
            ...
            catch (invalid_argument& ia) { err("stoi-im.pauseMapDebug"); }
...
{
    if (row >= sourceDim[1] || row < 0) { err("Row outside boundaries-im.pngExtractRow"); }
    vector<unsigned char> vRow;
...
void IMGFUNC::pngLoad(string& pathPNG)
{
    ...
    if (dataTemp == NULL)
    {
        ...
        if (dataTemp == NULL) { err("stbi_load-im.pngLoad"); }
    }
...
vector<vector<vector<int>>> IMGFUNC::pngThreeFrames()
{
    ...
    if (index < 1 || index > 3) { err("bad index-im.pngThreeFrames"); }
...
void IMGFUNC::pngToBin(SWITCHBOARD& sbgui, string& pathPNG, string& pathBIN)
{
    ...
    while (1)
    {
        ...
        {
            err("vBorderPath error-im.pngToBin");
        }
...
void IMGFUNC::pngToBinLive(SWITCHBOARD& sbgui, vector<vector<double>>& border)
{
    ...
    catch (invalid_argument& ia) { err("stod-im.pngToBinLive"); }
    double widthDemSup = (double)width / widthLabel;
    double heightDemSup = (double)height / heightLabel;
    if (widthDemSup > heightDemSup)
    {
    ...
    if (success)
    {
        ...
        if (!success) { err("sbgui.done-im.pngToBinLive"); }
        frameDone = 0;
        mycomm[1] = 1;
    ...
    while (1)
    {
        ...
        if (vBorderPath[vBorderPath.size() - 1][0] < 0 || vBorderPath[vBorderPath.size() - 1][1] < 0)
        {
            err("vBorderPath error-im.pngToBinLive");
        }
        sizeVBP++;
        searchRadiusIncrease = 0;
        rabbitHole = 0;
...
void IMGFUNC::pngToBinPause(SWITCHBOARD& sbgui)
{
    ...
    else { err("commGui-im.pngToBinPause"); }
...
int IMGFUNC::testCandidatesInteriorZone(SWITCHBOARD& sbgui, vector<vector<int>>& tracks, string sZone, vector<vector<int>>& candidates)
{
    ...
    if (candidates.size() != 2) { err("Not given two candidates-im.testCandidatesInteriorZone"); }
...
{
    if (candidates.size() != distances.size()) { err("Size mismatch-im.testDistances"); }
...
int IMGFUNC::testHereBeDragons(vector<vector<unsigned char>> Lrgb, vector<int>& borderCoord, unordered_map<string, int>& mapIndexCandidate)
{
    ...
    catch (out_of_range& oor) { err("mapIndexCandidate-im.testHereBeDragons"); }
...
int IMGFUNC::testZoneSweepLetters(vector<vector<int>>& zonePath, vector<vector<unsigned char>>& Lrgb, vector<vector<int>>& candidates, unordered_map<string, int>& mapIndexCandidate)
{
    ...
    for (int ii = 0; ii < candidates.size(); ii++)
    {
        ...
        catch (out_of_range& oor) { err("candidate map-im.testZoneSweepLetters"); }
        sZone = pixelZone(Lrgb[indexCandidate]);
        if (sZone != "white") { err("invalid candidate pixel-im.testZoneSweepLetters"); }
        ...
        while (1)
        {
            ...
            if (indexZone < 0)
            {
                if (crossOver > 0) { err("infinite loop-im.testZoneSweepLetters"); }
                crossOver = 1;
        ...
        while (1)
        {
            ...
            else if (sZone == "textHumanFeature")
            {
                ...
                else { err("no left boundary-im.testTextHumanFeature"); }
            }
            else
            {
                break;
...
void IMGFUNC::turnClockwise(vector<int>& dxdy)
{
	...
	{
		if (dxdy[0] != 0) { err("Input direction not NESW-im.turnClockwise"); }
		magnitude = -1 * dxdy[1];
		dxdy = { magnitude, 0 };
	}
	else if (dxdy[0] > 0)  // Normal force is eastward, so travel southward.
	{
		if (dxdy[1] != 0) { err("Input direction not NESW-im.turnClockwise"); }
		magnitude = dxdy[0];
		dxdy = { 0, magnitude };
	}
	...
	else { err("Cannot determine input direction-im.turnClockwise"); }
}
